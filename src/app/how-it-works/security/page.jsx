...
import { Lock, Eye, BrainCircuit, AlertTriangle, ShieldCheck } from 'lucide-react';
...
export default function SecurityMeasures() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-teal-600/10 blur-[100px] rounded-full pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto relative z-10 w-full">
        ...
        <div className="text-center mb-20">
          ...
          <p className="text-emerald-100/70 max-w-2xl mx-auto text-lg leading-relaxed">
            Industry-leading AI and manual proctoring measures working invisibly in the background to ensure absolute fairness for every assessment.
          </p>
        </div>
        ...
        >
          {[
            { icon: <Lock className="w-7 h-7"/>, title: "Browser Lock", desc: "Instantly locks the testing environment. Prevents new tabs, copying, pasting, or unauthorized external apps." },
            { icon: <Eye className="w-7 h-7"/>, title: "Webcam Tracking", desc: "Advanced AI monitors eye movement, tracks focal points, and detects multiple faces in real-time." },
            { icon: <BrainCircuit className="w-7 h-7"/>, title: "Audio Analysis", desc: "Intelligent sound monitoring detects background voices, whispering, or any suspicious environmental noise." },
            { icon: <AlertTriangle className="w-7 h-7"/>, title: "Screen Recording", desc: "Continuous high-res screen capture logs every action for manual review if an anomaly is flagged." }
          ].map((item, idx) => (
            <motion.div 
            ...
            >
              ...
              <p className="text-emerald-100/60 text-sm leading-relaxed flex-grow">
                {item.desc}
              </p>
            </motion.div>
          ))}
